...
public class ImageLoader {
    ...
    private void initDiskCache() {
        try {
            File cacheDir = OtherUtils.getDiskCacheDir(SugoodApplication.getInstance(), "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, OtherUtils.getAppVersion(SugoodApplication.getInstance()),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
